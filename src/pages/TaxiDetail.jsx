...
const TaxiDetail = () => {
    ...
    const item = transport.taxi.find(t => createSlug(t.name) === id);
    ...
    const features = [
        { icon: <FiShield />, title: 'Verified Drivers', desc: 'Background checked & trained' },
        { icon: <FiClock />, title: 'Flexible', desc: 'Custom stopovers allowed' },
        { icon: <FiUsers />, title: 'Privacy', desc: 'Private vehicle for your group' },
        { icon: <FiTruck />, title: 'Clean Cars', desc: 'Sanitized before every trip' }
    ];
            ...
            <div className="bg-gray-50 border-b border-gray-100">
                <div className="max-w-7xl mx-auto px-4 py-6">
                    <button
                        ...
                        className="flex items-center gap-2 text-gray-600 hover:text-yellow-600 transition font-medium mb-4"
                    >
                        <FiArrowLeft /> Back to Taxi Booking
                    </button>
                    <div className="flex items-center gap-3 text-sm">
                        <Link to="/" className="text-gray-500 hover:text-yellow-600 transition">Home</Link>
                        <span className="text-gray-300">/</span>
                        <Link to="/taxi-booking" className="text-gray-500 hover:text-yellow-600 transition">Taxi</Link>
                        <span className="text-gray-300">/</span>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                {item.images.map((img, idx) => (
                                    <button
                                        ...
                                        className={`aspect-square rounded-2xl overflow-hidden border-2 transition ${selectedImage === idx ? 'border-yellow-500 shadow-lg' : 'border-gray-200 hover:border-gray-300'}`}
                                    >
                    ...
                    <div className="space-y-8">
                        <div>
                            ...
                            {item.seats && (
                                <div className="flex items-center gap-2 text-gray-600 mb-6">
                                    ...
                                    <span className="font-semibold">{item.seats} Seater</span>
                                </div>
                            )}
                        </div>

                        {/* Pricing */}
                        <div className="bg-gradient-to-br from-yellow-500/5 to-yellow-500/10 rounded-3xl p-8 border border-yellow-500/20">
                            <p className="text-xs font-bold text-gray-600 uppercase tracking-widest mb-2">Detailed Pricing</p>
                            <div className="flex items-baseline gap-3">
                                <span className="text-5xl font-bold text-gray-900">
                                    ₹{item.pricePerKm}
                                </span>
                                <span className="text-xl text-gray-600 font-semibold">/km</span>
                            </div>
                            <p className="text-sm text-gray-600 mt-2 font-medium">Minimum billing distance applies per day.</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {features.map((feature, idx) => (
                                <div key={idx} className="bg-gray-50 p-6 rounded-2xl border border-gray-100 space-y-3">
                                    <div className="w-12 h-12 bg-yellow-500/10 text-yellow-500 rounded-xl flex items-center justify-center text-xl">
                                        {feature.icon}
                                    </div>
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 pt-6">
                            ...
                            <Link
                                to="/booking"
                                className="flex-1 bg-yellow-500 text-white py-4 rounded-xl font-bold text-center hover:bg-yellow-600 transition flex items-center justify-center gap-2 shadow-lg shadow-yellow-200"
                            >
                                <FiCheck /> Book Now
                            </Link>
                        </div>
