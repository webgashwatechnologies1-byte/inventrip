...
import { FiFilter, FiCompass, FiZap, FiActivity, FiRotateCcw, FiX } from 'react-icons/fi';
// Import a suitable hero image
import heroImage from '../assets/images/rainforest_hero_banner.png'; // Using a generic beautiful banner
...
const AllDestinations = () => {
    ...
    // Flatten all packages from all destinations into a single array
    const allPackages = useMemo(() => {
        return Object.values(destinations).flatMap(dest => dest.packages);
    }, []);

    const filteredPackages = useMemo(() => {
        let items = [...allPackages];
    ...
    }, [allPackages, activeSort, priceRange]);

    const handleImageError = (e) => {
        e.target.src = "https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1920";
    };
        ...
        <div className="min-h-screen bg-white font-poppins">
            ...
            <section className="relative h-[40vh] flex items-center overflow-hidden">
                <div className="absolute inset-0">
                    <img src={heroImage} alt="All Destinations" onError={handleImageError} className="w-full h-full object-cover scale-105" />
                ...
                <div className="relative max-w-7xl mx-auto px-4 w-full text-center sm:text-left">
                    ...
                    <div className="flex items-center justify-center sm:justify-start gap-6 text-white/50">
                        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
                            <FiZap className="text-tertiary" /> {allPackages.length} Packages
                        </div>
                        <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
                ...
                <div className="flex flex-col-reverse lg:flex-row gap-12">
                    ...
                    <div className="lg:w-1/4 space-y-8 mt-8 lg:mt-0 lg:sticky lg:top-8 lg:h-fit">
                        <div className="hidden lg:block">
                            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-6 border-l-4 border-[#39a34a] pl-3">Refine Search</h3>
                            <div className="space-y-8">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`text-left px-4 py-3 rounded-lg text-xs font-bold uppercase tracking-wide transition-all ${activeSort === s.id ? 'bg-primary text-white shadow-md' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}
                                            >
                                                {s.label}
                                            </button>
                                        ))}
                                ...
                                <button
                                    onClick={() => { setPriceRange(100000); setActiveSort('popular'); }}
                        ...
                        <div className="bg-[#0D2137] p-8 rounded-3xl text-white space-y-6 relative overflow-hidden shadow-2xl">
                            <div className="relative z-10">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/50 mb-2">Need Help?</p>
                                <h4 className="text-xl font-bold leading-tight mb-4">Talk to a Certified Expert</h4>
                                <a href="tel:7832911551" className="block text-2xl font-black tracking-tight hover:text-orange-500 transition-colors">+91 78329 11551</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:w-3/4">
                        <div className="flex items-center justify-between mb-8 border-b border-gray-100 pb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-lg font-bold text-gray-900 tracking-tight">Available Packages</h2>
                                </div>
                            </div>
                        ...
                        {filteredPackages.length === 0 && (
                            <div className="py-24 text-center space-y-6 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                                <div className="text-4xl text-gray-300 flex justify-center"><FiFilter /></div>
                                <h3 className="text-xl font-bold text-gray-600">No Packages Found</h3>
                                <button onClick={() => setPriceRange(100000)} className="text-primary font-bold uppercase tracking-widest text-xs border-b border-primary pb-0.5">Reset Filters</button>
                            </div>
                        )}
                    </div>
                </div>
            ...
            <div className={`fixed inset-0 z-50 lg:hidden transition-opacity duration-300 ${isMobileFilterOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                ...
                <div className={`absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl p-6 transition-transform duration-300 max-h-[85vh] overflow-y-auto ${isMobileFilterOpen ? 'translate-y-0' : 'translate-y-full'}`}>
                    ...
                    <div className="space-y-8">
                        ...
                        <button onClick={() => setIsMobileFilterOpen(false)} className="w-full bg-primary text-white py-4 rounded-xl font-bold uppercase tracking-widest text-sm shadow-lg">
